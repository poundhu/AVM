...
public class ArrayWrappingInterpreter extends BasicInterpreter{
    ...
    ArrayWrappingInterpreter(PocClassHierarchy hierarchy, boolean preserveDebuggability) {
      super(Opcodes.ASM6);
    ...
    public BasicValue merge(BasicValue value1, BasicValue value2) {
        return (this.preserveDebuggability)
            ? mergeWhenDebuggabilityIsPreserved(value1, value2)
            : mergeWhenDebuggabilityNotPreserved(value1, value2);
    }

    private BasicValue mergeWhenDebuggabilityIsPreserved(BasicValue value1, BasicValue value2) {
        ...
        if (b.equals(BasicValue.UNINITIALIZED_VALUE)) {
            ...
            if (dimension1 == 0 || dimension2 == 0) {
                return new BasicValue(Type.getType("[L" + Helpers.fulllyQualifiedNameToInternalName(PocClassInfo.SHADOW_OBJECT) + ";"));
            }

            //TODO: investigate this further -- also what about non-arrays? What actually matters here??
            if (dimension1 != dimension2) {
                return new BasicValue(Type.getType("[L" + Helpers.fulllyQualifiedNameToInternalName(PocClassInfo.SHADOW_IOBJECT_ARRAY) + ";"));
            }
    ...
    private BasicValue mergeWhenDebuggabilityNotPreserved(BasicValue value1, BasicValue value2) {
        BasicValue b = super.merge(value1, value2);
